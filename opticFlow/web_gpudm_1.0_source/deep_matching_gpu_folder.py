...
def get_net( image_shape, params, GPU=0, viz=()):
  net, levels = create_matching_net( image_shape, params, viz=viz )
  set_GPU(GPU)

  return net, levels


def get_img_shape( image_pairs, params):
  # prepare images
  if type(image_pairs[1])!=tuple: image_pairs=[image_pairs]
  num_pairs = len(image_pairs)
  image_pairs = [i for (p1,p2) in image_pairs for i in (p1,p2)]
...
def match_folder_images( image_pairs, params, net=None, GPU=0, viz=()):
  # prepare images
  if type(image_pairs[1])!=tuple: image_pairs=[image_pairs]
  num_pairs = len(image_pairs)
  image_pairs = [i for (p1,p2) in image_pairs for i in (p1,p2)]
  ...
  # build cnn if necessary
  if net is None:
    net, _ = create_matching_net( images.shape, params, viz=viz )
...
def parse_args():
  ...
  parser.add_argument("-f","--frame-folder", type=str, default="",
...
if __name__=='__main__':
  ...
  img_list = glob.glob(args.frame_folder + '/*')
  img_list.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))

  img0 = array(Image.open(img_list[0]).convert('RGB'))
  img1 = array(Image.open(img_list[1]).convert('RGB'))
  ...
  # launch matching
  image_shape = get_img_shape( (img0, img1), params)
  net, _ = get_net( image_shape, params, GPU=args.GPU, viz=viz)
  forward_file_name = os.path.join(args.output_folder, "forward_{}_{}.txt")
  backward_file_name = os.path.join(args.output_folder, "backward_{}_{}.txt")
